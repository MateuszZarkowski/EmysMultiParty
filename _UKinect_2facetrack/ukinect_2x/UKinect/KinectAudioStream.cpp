...
/// </summary>
KinectAudioStream::KinectAudioStream(IMediaObject *pKinectDmo, INuiAudioBeam * pNuiAudioSource) :
      m_cRef(1),
...
{
    pKinectDmo->AddRef();
	pNuiAudioSource->AddRef();
	m_pNuiAudioSource = pNuiAudioSource;
    ...
    while (bContinue)
    {
        ...
        do
        {
            ...
            else
            {
                ...
                if (SUCCEEDED(m_pNuiAudioSource->GetBeam(&beamAngle)) && SUCCEEDED(m_pNuiAudioSource->GetPosition(&sourceAngle, &sourceConfidence)))
                {
                    // Set reading to UVars
					beamAngle = (180.0 * beamAngle) / M_PI;
					sourceAngle = (180.0 * sourceAngle) / M_PI;
            ...
            if (cbProduced > 0)
            {
				...
				if (pWaveWriter && fRecording) pWaveWriter->WriteBytes(pbOutputBuffer, cbProduced);
...
bool KinectAudioStream::RecordStart(const std::string& fileName,const WAVEFORMATEX * WaveFormat)
{
	if (!fRecording){
		pWaveWriter = WaveWriter::Start(fileName, WaveFormat);
		fRecording = TRUE;
		if (pWaveWriter) return true;
		else return false;
	} else return false;
}

bool KinectAudioStream::RecordStop(){
	if (fRecording){
		if (pWaveWriter){
			pWaveWriter->Stop();
			delete pWaveWriter;
