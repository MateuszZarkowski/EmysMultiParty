...
USpeech::USpeech(const std::string& s) : urbi::UObject(s) {
  ...
  UBindVars(USpeech, visemeId, nextVisemeId, visemeTime, visemeTrig, isSpeaking, progress, voiceNo, volume, rate, pitch, debug);
...
int USpeech::init() {
  ...
  const ULONGLONG ullInterest = SPFEI(SPEI_VISEME) | SPFEI(SPEI_WORD_BOUNDARY) | SPFEI(SPEI_START_INPUT_STREAM) | SPFEI(SPEI_END_INPUT_STREAM);
    ...
    if (WAIT_OBJECT_0 == WaitForSingleObject(speechEvent, INFINITE)) {
      ...
      while (S_OK == spevent.GetFrom(Speech)) {
        ...
        case SPEI_VISEME:
          ...
          visemeTrig = 1;
          if (debug.as<bool>()) cout << "[USpeech] DEBUG: visemeID " << visemeId.as<int>() << " visemeTime " << visemeTime.as<int>() << " ms" << endl;
          break;

        case SPEI_WORD_BOUNDARY:
          if (startPosition == 0) startPosition = spevent.lParam;
          progress = (float)(spevent.lParam - startPosition) / speechLength;
          break;

        case SPEI_START_INPUT_STREAM:
          // for debug:
          if (debug.as<bool>()) cout << "[USpeech] DEBUG: start speaking " << endl;
          startPosition = 0;
          isSpeaking = true;
          break;
        ...
        case SPEI_END_INPUT_STREAM:
          // for debug:
          if (debug.as<bool>()) cout << "[USpeech] DEBUG: end speaking " << endl;
          progress = 1;
          isSpeaking = false;
          return;
        }
